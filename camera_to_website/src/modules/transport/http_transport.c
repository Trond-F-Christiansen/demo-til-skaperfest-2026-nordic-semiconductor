...
static int post_photo(const struct payload *payload)
{
	...
	sec_tag_t sec_tag = CONFIG_CAMERA_TO_WEBSITE_TRANSPORT_SEC_TAG;
	...
	err = getaddrinfo(CONFIG_CAMERA_TO_WEBSITE_TRANSPORT_HOSTNAME, HTTP_PORT, &hints, &result);
	if (err) {
	...
	err = setsockopt(socket_fd, SOL_TLS, TLS_HOSTNAME,
			 CONFIG_CAMERA_TO_WEBSITE_TRANSPORT_HOSTNAME,
			 strlen(CONFIG_CAMERA_TO_WEBSITE_TRANSPORT_HOSTNAME));
	if (err) {
		err = -errno;
		goto cleanup;
	}
	...
	request_length = snprintk(
		...
		"Connection: close\r\n\r\n",
		CONFIG_CAMERA_TO_WEBSITE_TRANSPORT_HOSTNAME,
		CONFIG_CAMERA_TO_WEBSITE_TRANSPORT_DEVICE_TOKEN,
	...
	if (strncmp(response, "HTTP/1.1 200", strlen("HTTP/1.1 200")) != 0 &&
		...
		LOG_ERR("Server rejected photo: %.32s", response);
		err = -EIO;
	}
...
static void transport_task(void)
{
	...
	while (!zbus_sub_wait(&transport, &channel, K_FOREVER)) {
		...
		err = post_photo(&payload);
		if (err) {
			LOG_ERR("Photo upload failed: %d", err);
		} else {
			LOG_INF("Uploaded %u byte photo", payload.len);
		}
	}
}

K_THREAD_DEFINE(transport_task_id, CONFIG_CAMERA_TO_WEBSITE_TRANSPORT_THREAD_STACK_SIZE,
