""" Snake with voice control input
Todo: 
 - eliminate possibility of doing multiple direction changes before each game tick
 - restart menu
 - easy/mid/hard modes
    - easy: no borders
    - mid: borders
...
from controller import SerialController, DIRECTIONS
...
class SNAKE:
    ...
    def move_snake(self):
        ...
        if self.new_block == True:
            body_copy = self.body[:] # last element is 'deleted'
...
while True:
    while not controller.directions.empty(): # voice conroll - drain queue before tick
    ...
    clock.tick(60) # how often we refresh (fps)
