 ...
 *
 * Only the subscribe topic is prefixed here. Publish topics are per-message
 * (e.g. "games/snake/score") and carried in the payload itself, built by the
 * uart_bridge, so there is no fixed publish topic to prepare.
 */
...
static void publish(struct payload *payload)
{
	...
	struct mqtt_publish_param param = {
		...
		.message.topic.topic.size = payload->topic_len,
	};
